...
static void* hs_fd_task(void* args)
{
	...
	while (HI_FALSE == priv->s_bStopSignal)
	{
		...
		if(0 == framecnt)
		{
			s32Ret = HI_MPI_SYS_MmzAlloc_Cached(&stDstData.au64PhyAddr[0], (HI_VOID**)&stDstData.au64VirAddr[0], "User", HI_NULL, Size0*3);  //640*640×3 
    	...
    	if(VIDEO_FRAME_FORMAT_YUV420SP_VU == frame->enPixelFormat)
    	{
			...
			s32Ret = HI_MPI_SYS_MmzFlushCache(stDstData.au64PhyAddr[0], (HI_VOID*)(size_t)stDstData.au64VirAddr[0], stDstData.u32Height*stDstData.au32Stride[0]*3);		  
			if(HI_SUCCESS != s32Ret)
			{ 					   
