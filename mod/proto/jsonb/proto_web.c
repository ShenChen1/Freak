...
}
{
    ...
    const size_t size_list_data[] = {WEB_ITEM_MAX,0};
    const int size_list_len = sizeof(size_list_data)/sizeof(size_t);
    jsonb_opt_array(opt, json_child, element->caps, sizeof(proto_web_svr_cap_t) * 1 * WEB_ITEM_MAX, size_list_data, size_list_len, 0, jsonb_opt_proto_web_svr_cap_t);
}
{
    cJSON *json_child = NULL;
    if (opt == JSONB_OPT_J2S) {
    ...
    const size_t size_list_data[] = {WEB_ITEM_MAX,0};
    const int size_list_len = sizeof(size_list_data)/sizeof(size_t);
    jsonb_opt_array(opt, json_child, element->cfgs, sizeof(proto_web_svr_cfg_t) * 1 * WEB_ITEM_MAX, size_list_data, size_list_len, 0, jsonb_opt_proto_web_svr_cfg_t);
}
}
