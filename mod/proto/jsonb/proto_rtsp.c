...
void jsonb_opt_proto_rtsp_svr_cfg_t(jsonb_opt_e opt, cJSON *json, void *e, size_t size)
{
    proto_rtsp_svr_cfg_t *element= e;
{
...
void jsonb_opt_proto_rtsp_svr_cap_t(jsonb_opt_e opt, cJSON *json, void *e, size_t size)
{
    proto_rtsp_svr_cap_t *element= e;
{
    cJSON *json_child = NULL;
    if (opt == JSONB_OPT_J2S) {
        if (cJSON_IsNull(json)) return;
...
void jsonb_opt_proto_rtsp_svr_t(jsonb_opt_e opt, cJSON *json, void *e, size_t size)
{
    proto_rtsp_svr_t *element= e;
{
    cJSON *json_child = NULL;
    if (opt == JSONB_OPT_J2S) {
        if (cJSON_IsNull(json)) return;
...
}
{
    ...
    const size_t size_list_data[] = {RTSP_ITEM_MAX,0};
    const int size_list_len = sizeof(size_list_data)/sizeof(size_t);
    jsonb_opt_array(opt, json_child, element->caps, sizeof(proto_rtsp_svr_cap_t) * 1 * RTSP_ITEM_MAX, size_list_data, size_list_len, 0, jsonb_opt_proto_rtsp_svr_cap_t);
}
{
    cJSON *json_child = NULL;
    if (opt == JSONB_OPT_J2S) {
    ...
    const size_t size_list_data[] = {RTSP_ITEM_MAX,0};
    const int size_list_len = sizeof(size_list_data)/sizeof(size_t);
    jsonb_opt_array(opt, json_child, element->cfgs, sizeof(proto_rtsp_svr_cfg_t) * 1 * RTSP_ITEM_MAX, size_list_data, size_list_len, 0, jsonb_opt_proto_rtsp_svr_cfg_t);
}
}
