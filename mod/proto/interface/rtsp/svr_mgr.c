...
static int __rtsp_svr_get(rtsp_svr_mgr_t *self, proto_rtsp_svr_cfg_t *cfg)
{
    ...
    proto_package_fill_header(ibuf, cfg->id, PROTO_RTSP_KEY_SVR, PROTO_ACTION_GET, priv->proto, isize);
    nnm_req_exchange(priv->req, ibuf, proto_package_size(ibuf), (void **)&obuf, &osize);
    assert(obuf->key == PROTO_RTSP_KEY_SVR);
    assert(!obuf->errcode);
...
static int __rtsp_svr_set(rtsp_svr_mgr_t *self, proto_rtsp_svr_cfg_t *cfg)
{
    ...
    proto_package_fill_header(ibuf, cfg->id, PROTO_RTSP_KEY_SVR, PROTO_ACTION_SET, priv->proto, isize);
    nnm_req_exchange(priv->req, ibuf, proto_package_size(ibuf), (void **)&obuf, &osize);
    assert(obuf->key == PROTO_RTSP_KEY_SVR);
    assert(!obuf->errcode);
...
static int __rtsp_svr_cap(rtsp_svr_mgr_t *self, proto_rtsp_svr_cap_t *cap)
{
    ...
    proto_package_fill_header(ibuf, cap->id, PROTO_RTSP_KEY_SVR, PROTO_ACTION_CAP, priv->proto, isize);
    nnm_req_exchange(priv->req, ibuf, proto_package_size(ibuf), (void **)&obuf, &osize);
    assert(obuf->key == PROTO_RTSP_KEY_SVR);
    assert(!obuf->errcode);
...
static int __rtsp_svr_num(rtsp_svr_mgr_t *self)
{
    ...
    proto_package_fill_header(ibuf, -1, PROTO_RTSP_KEY_SVR, PROTO_ACTION_NUM, priv->proto, isize);
    nnm_req_exchange(priv->req, ibuf, proto_package_size(ibuf), (void **)&obuf, &osize);
    assert(obuf->key == PROTO_RTSP_KEY_SVR);
    assert(!obuf->errcode);
