...
static int rtp_read(rtp_receiver_priv_t *priv, socket_t s)
{
    ...
    r = socket_recvfrom(s, priv->rtp_buffer, sizeof(priv->rtp_buffer), 0, (struct sockaddr *)&ss, &len);
    if (r < 12)
        return -1;
...
static int rtcp_read(rtp_receiver_priv_t *priv, socket_t s)
{
    ...
    struct sockaddr_storage ss;
    len = sizeof(ss);
    r   = socket_recvfrom(s, priv->rtcp_buffer, sizeof(priv->rtcp_buffer), 0, (struct sockaddr *)&ss, &len);
    if (r < 12)
        return -1;
...
static int rtp_receiver(rtp_receiver_priv_t *priv, socket_t rtp[2], int timeout)
{
    ...
    while (atomic_load32(&priv->loop)) {
        // RTCP report
        r = rtp_demuxer_rtcp(priv->demuxer, priv->rtcp_buffer, sizeof(priv->rtcp_buffer));
        if (r > 0)
            r = socket_sendto(rtp[1],
                              priv->rtcp_buffer,
                              r,
                              0,
...
rtp_udp_receiver_create(int rtp[2], const char *peer, int peerport[2], int payload, const char *encoding)
{
