                        ...
                        size_t num)
{
    ...
    if (RTSP_TRANSPORT_RTP_TCP == transport->transport) {
        ...
        snprintf(rtsp_transport,
                 sizeof(rtsp_transport),
                 "RTP/AVP;unicast;client_port=%hu-%hu;server_port=%hu-%hu%s%s",
                 transport->rtp.u.client_port1,
...
static int rtsp_onrecord(void *ptr,
                         ...
                         const double *scale)
{
    tracef("ptr:%p rtsp:%p uri:%s, session:%s, npt:%p, scale:%p", ptr, rtsp, uri, session, npt, npt);
