...
{
    int ret;
    proto_rtsp_svr_cfg_t cfg;

    proto_server_data_pre(param->format, jsonb_opt_proto_rtsp_svr_cfg_t, param->in, param->isize, &cfg, sizeof(proto_rtsp_svr_cfg_t));
    ...
    proto_server_data_post(param->format, jsonb_opt_proto_rtsp_svr_cfg_t, &cfg, sizeof(proto_rtsp_svr_cfg_t), param->out, param->osize);
    return ret;
}
...
{
    int ret;
    proto_rtsp_svr_cap_t cap;

    proto_server_data_pre(param->format, jsonb_opt_proto_rtsp_svr_cap_t, param->in, param->isize, &cap, sizeof(proto_rtsp_svr_cap_t));
    ...
    proto_server_data_post(param->format, jsonb_opt_proto_rtsp_svr_cap_t, &cap, sizeof(proto_rtsp_svr_cap_t), param->out, param->osize);
    return ret;
}
...
int msgbox_rtsp_svr(msgbox_param_t *param)
{
    int ret = 0;
